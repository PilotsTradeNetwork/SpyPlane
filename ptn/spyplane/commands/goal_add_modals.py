    ...
    async def on_submit(self, interaction: Interaction):
        ...
        try:
            ...
            if self.goalkind not in valid_goalkinds:
                await interaction.response.send_message(
                    f"❌ Invalid goal kind. Must be one of: {', '.join(valid_goalkinds)}",
                    ephemeral=True
                )
                return
            ...
            if self.goalkind == "RaiseInf" and not faction_other:
                await interaction.response.send_message(
                    "❌ Faction Other is required for RaiseInf goal kind.",
                    ephemeral=True
                )
                return
...
class AddCustomGoalModal(BaseGoalModal):
    ...
    async def on_submit(self, interaction: Interaction):
        ...
        try:
            ...
            await interaction.response.send_message(
                f"❌ Error adding custom goal: {str(e)}",
                ephemeral=True
            )
