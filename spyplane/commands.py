...
@app_commands.describe(
    system_name='Name of the system to track',
...
async def faction_track(interaction: Interaction, system_name: str, priority: str):
    ...
    if not is_valid:
        await interaction.followup.send(f"❌ **{system_name}** is not a valid system in our database.")
        return
    
    success = await repo.add_system(system_name, priority, interaction.user.name)
    
    if success:
        await interaction.followup.send(f"✅ Added **{system_name}** to tracking with **{priority}** priority")
    else:
        await interaction.followup.send(f"❌ Failed to add **{system_name}**. It may already be tracked.")


@bot.tree.command(name='faction_remove')
@app_commands.describe(
    system_name='Name of the system to remove from tracking'
)
async def faction_remove(interaction: Interaction, system_name: str):
    ...
    if success:
        ...
        await interaction.followup.send(f"❌ **{system_name}** was not found in tracked systems")
...
async def faction_list(interaction: Interaction):
    ...
    for system in tracked_systems:
        added_date = datetime.fromtimestamp(system.added_at).strftime('%Y-%m-%d %H:%M:%S')
