...
class SystemsPostingService:
    ...
    async def publish_systems_to_scout(self):
        ...
        log(f"Posting systems - Primary: {len(splits['Primary'])}, Secondary: {len(splits['Secondary'])}, Tertiary: {len(splits['Tertiary'])}")
    ...
    async def post_list(self, splits, priority_string):
        ...
        if len(systems_for_priority):
            ...
            if priority_string == 'Primary':
                ...
                systems_to_post = every_other_day[daily_sequence % 2] if every_other_day else []
                ...
                systems_to_post = every_third_day[daily_sequence % 3] if every_third_day else []
            else:
                systems_to_post = systems_for_priority
            
            # Log the actual count that will be posted after rotation
            log(f"Posting {len(systems_to_post)} {priority_string} systems (day {daily_sequence})")
    ...
    def split_systems_by_priority(self, systems: List[ScoutSystem], carryover: List[ScoutSystem]) -> Dict[str, List[ScoutSystem]]:
        ...
        return {
            ...
            'Secondary': splits['Secondary'] + [s for s in carryover if s.priority == 'Secondary' and s.system not in [l.system for l in splits['Secondary']]],
            'Tertiary': splits['Tertiary'] + [s for s in carryover if s.priority == 'Tertiary' and s.system not in [l.system for l in splits['Tertiary']]]
        }
