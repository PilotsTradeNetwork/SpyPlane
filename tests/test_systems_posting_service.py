...
class SystemsPostingServiceTests(unittest.TestCase):
    ...
    def test_split_systems_by_priority(self):
        primary = [ScoutSystem(f"System{i}", "Primary", "test_user", 1) for i in range(1, 5)]
        secondary: list[ScoutSystem] = [ScoutSystem(f"System{i}", "Secondary", "test_user", 1) for i in range(6, 12)]
        tertiary = [ScoutSystem(f"System{i}", "Tertiary", "test_user", 1) for i in range(12, 21)]
        carryover = []
        ...
        carryover_secondary = [ScoutSystem("CarryoverSecondary", "Secondary", "test_user", 1)]
        carryover_tertiary = [ScoutSystem("CarryoverTertiary", "Tertiary", "test_user", 1)]
        carryover = carryover_secondary + carryover_tertiary

        day_list_with_carryover = self.subject.split_systems_by_priority(systems, daily_sequence, carryover)
        ...
        self.assertGreaterEqual(len(day_list_with_carryover["Secondary"]), secondary_count_before_carryover)
        self.assertGreaterEqual(len(day_list_with_carryover["Tertiary"]), tertiary_count_before_carryover)
